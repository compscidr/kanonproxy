...
@Composable
fun PermissionScreen(kAnonViewModel: KAnonViewModel) {
    ...
    val startForResult =
        rememberLauncherForActivityResult(ActivityResultContracts.StartActivityForResult()) { result: ActivityResult ->
            if (result.resultCode == Activity.RESULT_OK) {
                ...
                kAnonViewModel.hidePermissionScreen()
            } else {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        Row(modifier = Modifier.padding(start = 24.dp, top = 54.dp, end=24.dp)) {
            Text("This app uses the VPN service to capture packets so that you can analyze them.")
        }
...
fun PermissionScreenPreview() {
    val context = LocalContext.current
    val sessionViewModel = KAnonViewModel.getInstance(PreferenceManager.getDefaultSharedPreferences(context))
    PermissionScreen(sessionViewModel)
}